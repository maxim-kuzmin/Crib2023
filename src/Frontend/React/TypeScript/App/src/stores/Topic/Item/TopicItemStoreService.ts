...
type Data = TopicDomainItemGetOperationResponse;
...
async function runDispatchToLoad (
  requestHandler: TopicDomainItemGetOperationRequestHandler,
  dispatch: Dispatch<Action>,
  callback: CallbackToSet | null,
...
function useDispatchToLoad (options?: DispatchOptionsToLoad): DispatchToLoad {
  ...
  const requestHandler = useRef(getModule().useTopicDomainItemGetOperationRequestHandler()).current;
...
export interface TopicItemStoreService {
...
export function createTopicItemStoreService (): TopicItemStoreService {
  return {
    DispatchContext,
    StateContext,
    initialState,
